#############################################
#### INTERACTIONS WITH AGE IN DTI SAMPLE ####
#############################################
...
dtiTrGam <- lapply(dtiTrRegions, function(x) {
  gam(substitute(i ~ s(age) + sex + dti64Tsnr + Hydra_k2 + Hydra_k2*age, list(i = as.name(x))), method="REML", data=subjData)
})
...
#Pull the p-values
dtiTr_p <- sapply(dtiTrAnova, function(v) v$pTerms.pv[5])
