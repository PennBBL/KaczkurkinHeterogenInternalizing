##########################################################
#### INTERACTIONS WITH GENDER IN VOLUME/CT JLF SAMPLE ####
##########################################################
...
ctGam <- lapply(ctRegions, function(x) {
  gam(substitute(i ~ s(age) + sex + averageManualRating + Hydra_k2 + Hydra_k2*sex, list(i = as.name(x))), method="REML", data=subjData)
})
