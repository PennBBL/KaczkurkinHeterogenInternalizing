##########################################################
#### INTERACTIONS WITH GENDER IN RESTING STATE SAMPLE ####
##########################################################
...
restAlffGam <- lapply(restAlffRegions, function(x) {
  gam(substitute(i ~ s(age) + sex + restRelMeanRMSMotion + Hydra_k2 + Hydra_k2*sex, list(i = as.name(x))), method="REML", data=subjData)
})
