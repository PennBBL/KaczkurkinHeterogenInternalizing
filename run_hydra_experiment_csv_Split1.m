function [CIDX,ARI] = run_hydra_experiment_csv_Split1(featureCSV,outputDir,varargin)
...
YNC = Y(Y==-1); XKNC = XK(Y==-1,:); IDNC = ID(Y==-1);
YP = Y(Y==1); XKP = XK(Y==1,:);IDP = ID(Y==1);

%% split begin
IDXNC = ceil(size(YNC,1)/2 +1):size(YNC,1);% ~half of controls
YNCf = YNC(IDXNC);
XKNCf = XKNC(IDXNC,:);
IDNCf = IDNC(IDXNC);

IDXP = 1:ceil(size(YP,1)/2);% ~half of patients
YPf = YP(IDXP);
XKPf = XKP(IDXP,:); 
IDPf = IDP(IDXP);
...
disp('Saving results...')
if(params.vo==0)
    save([outputDir '/HYDRA_results_Split1.mat'],'ARI','CIDX','clustering','ID','model');
else
    save([outputDir '/HYDRA_results_Split1.mat'],'ARI','CIDX','clustering','ID','XK','Y','covar','IDcovar','model');
end
disp('Done')
end
