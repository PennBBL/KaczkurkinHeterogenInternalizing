function [CIDX,ARI] = run_hydra_experiment_csv_Split2(featureCSV,outputDir,varargin)
...
%% split begin
IDXNC = 1:ceil(size(YNC,1)/2);% ~half of controls
YNCf = YNC(IDXNC);
XKNCf = XKNC(IDXNC,:);
IDNCf = IDNC(IDXNC);

IDXP = ceil(size(YP,1)/2 +1):size(YP,1);% ~half of patients
YPf = YP(IDXP);
XKPf = XKP(IDXP,:); 
IDPf = IDP(IDXP);
...
disp('Saving results...')
if(params.vo==0)
    save([outputDir '/HYDRA_results_Split2.mat'],'ARI','CIDX','clustering','ID','model');
else
    save([outputDir '/HYDRA_results_Split2.mat'],'ARI','CIDX','clustering','ID','XK','Y','covar','IDcovar','model');
end
disp('Done')
end
