function [CIDX,ARI] = run_hydra_experiment_csv_partialNCPatients(featureCSV,outputDir,varargin)
...
%% Begin ~50% controls and other ~50% patients 
YNC = Y(Y==-1); XKNC = XK(Y==-1,:); 
nc1 = length(YNC); nc2 = ceil(size(YNC,1)/2);
IDXNC = randperm(nc1,nc2);
YNCf = YNC(IDXNC);
XKNCf = XKNC(IDXNC,:);

YSZ = Y(Y==1); XKSZ = XK(Y==1,:);
pt = length(YSZ); pp = ceil(size(YNC,1)/2);
IDXSZ = randperm(pt,pp);
YSZf = YSZ(IDXSZ);
XKSZf = XKSZ(IDXSZ,:);

Y = cat(1,YNCf,YSZf);
XK = cat(1,XKNCf,XKSZf);
